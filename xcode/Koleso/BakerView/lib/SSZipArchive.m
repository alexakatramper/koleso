...
//------------------------------------------------------------------------------
// assume that zipped file conteins only one html file that would be decompressed into the string
// anyway only the first entry will bw unzipped
//------------------------------------------------------------------------------
+ (BOOL)unzipFileAtPath:(NSString *)path toString:(NSString **)destination password:(NSString *)password error:(NSError **)error {
	// Begin opening
	zipFile zip = unzOpen((const char*)[path UTF8String]);
	if (zip == NULL) {
		if (error) {
	...
	@autoreleasepool {
		...
		if (ret == UNZ_OK) {
			...
			ret = unzGetCurrentFileInfo(zip, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
			if (ret == UNZ_OK) {
				tmpStr = (char*)malloc( fileInfo.uncompressed_size + 1 );
				
				int readBytes = unzReadCurrentFile(zip, tmpStr, fileInfo.uncompressed_size);
				tmpStr[fileInfo.uncompressed_size] = 0;
				if (readBytes == fileInfo.uncompressed_size) {
					*destination = [[NSString stringWithCString:tmpStr encoding:NSUTF8StringEncoding] retain];
					success = YES;
				}
